...
/**
  * @brief  IIC写1字节（底层封装）
  */
static int8_t MPU6050_I2C_Write(uint8_t reg, uint8_t data) {
    return HAL_I2C_Mem_Write(&hi2c1, MPU6050_ADDR_7BIT<<1, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 100) == HAL_OK ? 0 : -1;
}

/**
  * @brief  IIC读多字节（底层封装）
  */
static int8_t MPU6050_I2C_Read(uint8_t reg, uint8_t *data, uint16_t len) {
    return HAL_I2C_Mem_Read(&hi2c1, MPU6050_ADDR_7BIT<<1, reg, I2C_MEMADD_SIZE_8BIT, data, len, 100) == HAL_OK ? 0 : -1;
}
...
int8_t MPU6050_Init(void) {
    ...
    // 1. 复位设备（REG_PWR_MGMT_1第7位置1）
    if (MPU6050_I2C_Write(REG_PWR_MGMT_1, 0x80) != 0) return -1;
    HAL_Delay(100); // 手册要求复位后稳定100ms

    // 2. 检测设备ID
    if (MPU6050_I2C_Read(REG_WHO_AM_I, &who_am_i, 1) != 0 || who_am_i != MPU6050_WHO_AM_I_VAL) {
        return -1;
    }

    // 3. 配置寄存器（按手册时序，每步等待5ms稳定）
    MPU6050_I2C_Write(REG_SMPLRT_DIV, 0x02);    // 采样率=1000/(2+1)≈333Hz
    HAL_Delay(5);
    MPU6050_I2C_Write(REG_CONFIG, 0x03);      // DLPF=3（42Hz带宽，低通滤波）
    HAL_Delay(5);
    MPU6050_I2C_Write(REG_GYRO_CONFIG, 0x18); // 陀螺仪±2000°/s（FS_SEL=3）
    HAL_Delay(5);
    MPU6050_I2C_Write(REG_ACCEL_CONFIG, 0x08); // 加速度计±4G（AFS_SEL=1）
    HAL_Delay(5);
    MPU6050_I2C_Write(REG_PWR_MGMT_1, 0x03);  // 时钟源=陀螺仪Z轴（手册推荐提升稳定性）
    HAL_Delay(5);
